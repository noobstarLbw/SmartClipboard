...
- 🍃 **零轮询超低能耗**：基于 Windows 原生 `AddClipboardFormatListener` 事件通知机制，拒绝低效定时轮询，空闲时 **0.0% CPU 占用**，内存仅占用约 **25MB ~ 30MB**。
- 🛠️ **系统托盘与开机自启**：Windows 任务栏右下角常驻托盘图标，支持随时一键暂停/恢复拦截、清空历史、开机自启动开关以及安全退出。
